...
const UserUrl = () => {
  ...
  if (isError) {
    return (
      <div className="bg-red-900 border border-red-500 text-red-200 px-4 py-3 rounded my-4">
  ...
  if (!urls.urls || urls.urls.length === 0) {
    return (
      <div className="text-center text-gray-300 my-6 p-4 bg-gray-900 rounded-lg border border-gray-600">
  ...
  return (
    <div className="bg-slate-900 rounded-lg mt-5 shadow-md overflow-hidden border border-teal-600 border-opacity-40 hover:border-opacity-60 hover:shadow-teal-500/20 transition-all duration-300">
      
      <div className="overflow-x-auto h-56">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-slate-800 border-b border-teal-700">
            <tr>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-teal-300 uppercase tracking-wider">
                Original URL
              </th>
          ...
          <tbody className="bg-slate-900 divide-y divide-teal-700">
            {urls.urls.reverse().map((url) => (
              <tr key={url._id} className="hover:bg-slate-800 transition-colors">
                <td className="px-6 py-4">
                  <div className="text-sm text-emerald-100 truncate max-w-xs">
                    {url.full_url}
                  </div>
                </td>
                <td className="px-6 py-4">
